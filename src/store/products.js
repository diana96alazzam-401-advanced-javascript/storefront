...
export default (state = initialState, action) => {
  ...
  switch (type) {
  case 'ACTIVATE':
    let filteredProducts = initialState.products.filter(item=> {
  ...
  case 'ADD':
    let mappedProducts = state.products.map(item => {
      if (item.name === payload.name){
        return {
          name: item.name, category: item.category, price: item.price, inStock: (payload.inStock-1),
          image: item.image,
        };
      }
      return item;
